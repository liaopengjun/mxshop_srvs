...
func VerifyPwd(pwd, rawPwd string) bool {
	...
	checkPwd := pwdSlice[3]
	options := &password.Options{16, 100, 32, sha512.New}
	return password.Verify(rawPwd, salt, checkPwd, options)
}
...
func main() {
	...
	// 生成用户的md5密码
	newPassword := GeneratePwd("admin@123!")

	// 校验密码
	fmt.Println(VerifyPwd(newPassword, "admin@123!"))
